...
const index$1 = require('./index-eac73f27.js');
...
const CalendarColumnViewBackground = class {
  constructor(hostRef) {
  ...
  renderBackgroundColumns() {
    ...
    for (let i = 0; i < this.columns; i++) {
      const cls = ['column'];
      if (index$1.isEqual(index$1.startOfDay(i), index$1.startOfDay(new Date())))
        cls.push('active');
      columns.push(index.h("div", { class: cls.join(' ') }));
    }
    return columns;
  }
  render() {
    ...
    for (let i = 0; i < 48; i++) { // @TODO: parameterize number of column, currently rendering 24 hours
      ...
      if (i % 2)
        cls.push('hour');
      const row = index.h("div", { class: cls.join(' ') }, this.renderBackgroundColumns());
      rows.push(row);
    }
    return (index.h(index.Host, null, index.h("div", { class: 'background' }, rows)));
  }
};
CalendarColumnViewBackground.style = columnViewBackgroundCss;

exports.goat_calendar_column_view_background = CalendarColumnViewBackground;

//# sourceMappingURL=goat-calendar-column-view-background.cjs.entry.js.map